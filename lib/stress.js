...
const TOKENS = new SharedArray("tokens", function () {
  ...
  // Text file (one token per line) or any TOKENS_FILE
...
export default async function () {
  ...
  if (!token) {
    console.error("❌ TOKEN or TOKENS_FILE required. Pass via -e TOKEN=<jwt> or -e TOKENS_FILE=tokens.json");
    return;
  }
  ...
  try {
    ...
    await tracked("wishlist_add", page, async () => {
      ...
      if (!added) {
        // Try SVG heart icon or any heart-shaped button
        added = await safeClick(
