...
// ─── TOKEN HANDLING ─────────────────────────────────────
// Single token:  -e TOKEN=<jwt>
// Multi token:   -e TOKENS_FILE=tokens.json
//   tokens.json = ["token1", "token2", "token3"]
let TOKENS = [];
if (__ENV.TOKENS_FILE) {
  TOKENS = JSON.parse(open(__ENV.TOKENS_FILE));
  console.log(`Loaded ${TOKENS.length} tokens from ${__ENV.TOKENS_FILE}`);
} else if (__ENV.TOKEN) {
  TOKENS = [__ENV.TOKEN];
}
...
function getToken() {
  ...
  // Each VU gets a token based on its VU number (round-robin)
