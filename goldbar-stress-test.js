...
// Option 2: Text file         → -e TOKENS_FILE=tokens.txt  (one token per line)
// Option 3: CSV file          → -e TOKENS_CSV=users.csv    (header: token)
// Option 4: JSON file         → -e TOKENS_JSON=tokens.json
const TOKENS = new SharedArray("tokens", function () {
  if (__ENV.TOKENS_CSV) {
    const csv = papaparse.parse(open(__ENV.TOKENS_CSV), { header: true });
    const tokens = csv.data.map((row) => row.token).filter(Boolean);
    console.log(`Loaded ${tokens.length} tokens from CSV: ${__ENV.TOKENS_CSV}`);
    return tokens;
  }
  if (__ENV.TOKENS_JSON) {
    const tokens = JSON.parse(open(__ENV.TOKENS_JSON));
    console.log(`Loaded ${tokens.length} tokens from JSON: ${__ENV.TOKENS_JSON}`);
    return tokens;
  }
  if (__ENV.TOKENS_FILE) {
    const raw = open(__ENV.TOKENS_FILE);
    const tokens = raw.split("\n").map((t) => t.trim()).filter(Boolean);
    console.log(`Loaded ${tokens.length} tokens from file: ${__ENV.TOKENS_FILE}`);
    return tokens;
  }
  if (__ENV.TOKEN) {
    return [__ENV.TOKEN];
...
export default async function () {
  ...
  if (!token) {
    console.error("❌ TOKEN or TOKENS_FILE required. Pass via -e TOKEN=<jwt> or -e TOKENS_FILE=tokens.txt");
    return;
  }
  ...
  try {
    ...
    await tracked("login", page, async () => {
      await page.goto(`${BASE}/sso/callback?token=${token}`, {
        waitUntil: "load",
        timeout: TIMEOUT,
      });
      await page.waitForLoadState("domcontentloaded");
    ...
    await tracked("wishlist_add", page, async () => {
      ...
      if (!added) {
        added = await safeClick(
          page.locator('[class*="wishlist"], [class*="heart"], [class*="favorite"]'),
          3000
        );
      }
      if (added) {
...
export function handleSummary(data) {
  return {
    "goldbar-stress-report.html": htmlReport(data),
    "goldbar-stress-summary.json": JSON.stringify(data, null, 2),
